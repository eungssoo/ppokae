...
export const RankingBoardView: React.FC<RankingBoardViewProps> = ({
  ...
  return (
    <div className="min-h-screen bg-slate-950 bg-animated-gradient flex items-center justify-center p-4 sm:p-6 md:p-8 selection:bg-indigo-500 selection:text-white">
      <div className="max-w-2xl w-full glass-card rounded-[2.5rem] p-6 sm:p-10 relative border border-slate-700/60 shadow-2xl overflow-hidden text-left">
        
        {/* Back Button */}
        <button
          onClick={onBack}
          className="mb-5 text-slate-300 hover:text-white font-bold transition-all flex items-center gap-2 bg-slate-800/80 hover:bg-slate-700/80 px-4 py-2 rounded-xl shadow-sm border border-slate-700 text-sm active:scale-95"
        >
        ...
        {isCycleEnded && userRank && !isClaimed && cycleReward && (
          <div className="mb-5 p-4 sm:p-5 rounded-3xl bg-gradient-to-r from-amber-500/25 via-yellow-500/20 to-orange-500/25 border-2 border-amber-400 shadow-md flex items-center justify-between gap-3 animate-pulse">
        ...
        {isCycleEnded && isClaimed && userRank && (
          <div className="mb-5 p-3 rounded-2xl bg-slate-800/80 border border-slate-700 text-center text-xs font-bold text-slate-300">
        ...
        {onOpenGachaModal && (
          <button
            ...
            className="w-full mb-4 p-3 sm:p-3.5 rounded-2xl bg-gradient-to-r from-purple-950/80 via-pink-950/70 to-slate-900 border border-purple-500/40 hover:border-purple-400 text-white font-bold text-xs sm:text-sm flex items-center justify-between shadow-md transition-all active:scale-[0.99] group"
          >
        ...
        {/* 🏆 회차별 차등 순위 보상표 */}
        <div className="bg-slate-900/90 border border-indigo-500/30 rounded-2xl p-3.5 mb-5 shadow-sm">
        ...
        {rankingData.length === 0 ? (
          <div className="bg-slate-900/60 rounded-3xl border-2 border-dashed border-slate-750 p-8 sm:p-10 text-center text-slate-400">
          ...
          <div className="space-y-2.5 max-h-[420px] overflow-y-auto pr-1">
            {rankingData.map((rank, index) => {
              ...
              let badge: React.ReactNode = (
                <span className="w-7 h-7 flex items-center justify-center bg-slate-800 rounded-full text-slate-300 font-bold text-xs border border-slate-700 shrink-0">
                  {index + 1}
                </span>
              );
              let rowStyle = 'bg-slate-850 border-slate-750 hover:border-slate-600';
              let textStyle = 'text-slate-200';

              if (index === 0) {
                badge = <span className="text-2xl sm:text-3xl filter drop-shadow shrink-0">🥇</span>;
                rowStyle = 'bg-amber-500/20 border-amber-400/50 shadow-md ring-1 ring-amber-400/30';
                textStyle = 'text-amber-300 font-black';
              } else if (index === 1) {
                badge = <span className="text-2xl sm:text-3xl filter drop-shadow shrink-0">🥈</span>;
                rowStyle = 'bg-slate-700/40 border-slate-500/40';
                textStyle = 'text-slate-100 font-black';
              } else if (index === 2) {
                badge = <span className="text-2xl sm:text-3xl filter drop-shadow shrink-0">🥉</span>;
                rowStyle = 'bg-orange-950/40 border-orange-700/40';
                textStyle = 'text-orange-200 font-black';
              }
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className={`w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-gradient-to-br ${rank.avatarBgGradient || 'from-slate-700 to-slate-800 border-slate-600'} border flex items-center justify-center text-xl sm:text-2xl shadow-sm shrink-0 relative`}>
