...
export const QuestionReportModal: React.FC<QuestionReportModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.error || '신고 접수 중 오류가 발생했습니다.');
    }
  };
      ...
      <div className="max-w-lg w-full glass-card rounded-[2.5rem] p-5 sm:p-7 border border-rose-500/40 shadow-2xl relative overflow-hidden text-left">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">
              채택 시 <strong className="text-amber-300">🪙 +50 코인 보상금</strong>이 지급됩니다! (일일 최대 10건)
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/90 rounded-2xl p-3.5 border border-slate-800 mb-4 text-xs">
          <div className="flex items-center justify-between text-slate-400 font-bold mb-1.5">
            <span className="text-purple-300 font-black">문항 #{question.form}형식</span>
            <span className="text-emerald-400 font-black">정답: {question.answer}</span>
          </div>
          <p className="text-white font-bold text-sm mb-1">{question.sentence}</p>
          <p className="text-slate-400">{question.translation}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1">
              상세 의견 및 수정 제안 <span className="text-rose-400">*</span>
            </label>
            <textarea
              rows={3}
              value={feedback}
              onChange={(e) => setFeedback(e.target.value)}
              placeholder="예: 보기 2번이 문법적으로 맞는 이유나 번역이 어색한 부분을 구체적으로 적어주세요. 깐깐한 AI 검수위원이 4단계로 심사합니다."
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              ...
              className="flex-[2] py-3 bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600 disabled:opacity-40 text-white rounded-xl font-black text-xs transition-all shadow-md active:scale-95 flex items-center justify-center gap-1.5"
            >
              <Send className="w-3.5 h-3.5" />
              <span>{isSubmitting ? '신고 접수 중...' : '제보 접수하기'}</span>
            </button>
          </div>
        </form>
