...
export const getExpressionCategories = (language: 'ko' | 'en'): ExpressionCategoryInfo[] => {
  if (language === 'en') {
    return [
      {
        id: 'daily',
        title: 'Daily Casual & Idioms',
        subTitle: 'Netflix & Everyday Life',
        icon: '☕',
        badge: 'HOT',
        gradient: 'from-amber-500/20 via-orange-500/10 to-slate-800/80 border-orange-500/40 hover:border-orange-400',
        desc: 'Vibrant native idioms and slang used daily in shows, movies, and casual talks.'
      },
      {
        id: 'business',
        title: 'Business & Office English',
        subTitle: 'Workplace & Meeting Mastery',
        icon: '💼',
        badge: 'WORK',
        gradient: 'from-blue-500/20 via-indigo-500/10 to-slate-800/80 border-indigo-500/40 hover:border-indigo-400',
        desc: 'Essential professional expressions for global business, email communications, and negotiations.'
      },
      {
        id: 'travel',
        title: 'Travel & Dining & Shopping',
        subTitle: 'Airport, Hotel & Restaurant',
        icon: '✈️',
        badge: 'TRIP',
        gradient: 'from-emerald-500/20 via-teal-500/10 to-slate-800/80 border-emerald-500/40 hover:border-emerald-400',
        desc: 'Confident, practical English for airports, hotels, restaurants, and shopping.'
      },
      {
        id: 'pattern',
        title: 'Native Power Patterns 100',
        subTitle: 'Core Speaking Formulas',
        icon: '🎬',
        badge: 'BEST',
        gradient: 'from-purple-500/20 via-pink-500/10 to-slate-800/80 border-purple-500/40 hover:border-purple-400',
        desc: 'Magical conversational formulas to effortlessly build 100+ native sentences.'
      }
    ];
  }
...
export const ExpressionSelectView: React.FC<ExpressionSelectViewProps> = ({
  ...
  const categories = getExpressionCategories(language);
      ...
      <div className="max-w-2xl w-full glass-card rounded-[2.5rem] p-6 sm:p-10 relative border border-slate-700/60 shadow-2xl">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl sm:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-300 to-amber-300 tracking-tight pb-1">
            {language === 'en' ? 'Master Native Expressions 🌟' : '실전 원어민 표현 마스터 🌟'}
          </h2>
          <p className="text-slate-400 text-xs sm:text-sm font-medium mt-1">
            {language === 'en' 
              ? 'Choose a study mode or generate 5 new expressions with AI (🪙 50)!'
              : '학습 모드를 선택하거나, AI로 새로운 실전 표현 5개(🪙 50)를 생성해 보세요!'}
          </p>
        </div>
                  ...
                  <div className="flex justify-between items-center mb-2.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="bg-purple-500/20 text-purple-200 border border-purple-500/40 text-[11px] font-black px-2.5 py-0.5 rounded-full flex items-center gap-1">
                        ...
                        <span>{count} {language === 'en' ? 'Items' : '개 보유'}</span>
                      </span>
                ...
                <div className="space-y-2 pt-2 border-t border-white/10">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      <span>{language === 'en' ? 'Flashcards' : '플래시카드'}</span>
                    </button>
                    ...
                    <button
                      ...
                      className="py-2.5 px-3 bg-gradient-to-r from-purple-500/80 to-pink-500/80 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl text-xs font-bold transition-all border border-purple-400/40 flex items-center justify-center gap-1.5 active:scale-95 shadow-sm"
                    >
                      <PenTool className="w-3.5 h-3.5" />
                      <span>{language === 'en' ? 'Quiz' : '퀴즈 풀기'}</span>
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-2 px-3 bg-white/5 hover:bg-white/10 text-purple-200 hover:text-white rounded-xl text-xs font-bold transition-all border border-white/10 flex items-center justify-center gap-1.5 active:scale-95 disabled:opacity-50"
                  >
                    <Plus className="w-3.5 h-3.5 text-pink-400" />
                    <span>{language === 'en' ? 'AI Generate 5 Expressions (🪙 50)' : 'AI 표현 5개 추가 생성 (🪙 50)'}</span>
                  </button>
                </div>
              </div>
            );
