      ...
      <div className="max-w-3xl w-full glass-card rounded-[2.5rem] p-5 sm:p-8 relative border border-slate-700/60 shadow-2xl text-left">
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-700/90 rounded-2xl text-slate-900 dark:text-white text-xs sm:text-sm placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all font-medium shadow-sm"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
              >
                ✕
              </button>
            )}
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
                selectedCategoryId === 'all'
                  ...
                  : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800'
              }`}
            >
        ...
        <div className="space-y-6">
          {filteredCategories.length === 0 ? (
            <div className="bg-white dark:bg-slate-900/60 rounded-3xl p-10 border border-slate-200 dark:border-slate-800 text-center text-slate-500 dark:text-slate-400 shadow-sm">
              <Search className="w-8 h-8 mx-auto text-slate-400 mb-2" />
              <p className="font-black text-sm text-slate-900 dark:text-white mb-1">
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-2xl border transition-all duration-200 overflow-hidden shadow-sm ${
                          isExpanded
                            ? 'bg-white dark:bg-slate-900/90 border-amber-400 dark:border-amber-500/40 shadow-md'
                      ...
                      >
                        ...
                        <button
                          ...
                          className="w-full p-4 flex items-center justify-between text-left gap-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/40"
                        >
                          <div className="flex items-center gap-3">
                            <span className="w-8 h-8 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-700 dark:text-amber-300 flex items-center justify-center text-xs font-black flex-shrink-0">
                              ⚡
                            </span>
                            <div>
                              <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                                ...
                                <span className="bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-500/40 px-2 py-0.2 rounded-full text-[10px] font-black">
                                  {badge}
                                </span>
                              </div>
                          ...
                          <div className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1 rounded-lg bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 flex-shrink-0">
                        ...
                        {isExpanded && (
                          <div className="px-4 pb-4 pt-1 space-y-3.5 text-xs text-slate-700 dark:text-slate-300 border-t border-slate-100 dark:border-slate-800/80">
                            ...
                            {/* 📌 Formula Box */}
                            {formula && (
                              <div className="bg-amber-50 dark:bg-amber-500/10 p-3.5 rounded-xl border border-amber-300 dark:border-amber-500/30">
                                <div className="text-[10px] font-black uppercase text-amber-800 dark:text-amber-400 tracking-wider mb-1 flex items-center gap-1">
                                  <Zap className="w-3 h-3 text-amber-600" />
                                  <span>{language === 'en' ? 'Core Formula' : '핵심 공식'}</span>
                                </div>
                                <div className="font-mono text-xs sm:text-sm font-black text-amber-950 dark:text-amber-200">
                                  {formula}
                                </div>
                              </div>
                            )}
                            ...
                            {tip.examples && tip.examples.length > 0 && (
                              <div className="space-y-2">
                                <div className="text-[11px] font-black text-slate-700 dark:text-slate-400 flex items-center gap-1">
                                  <span>🎯 {language === 'en' ? 'Real Example & Breakdown:' : '실전 예문 & 오답 분석:'}</span>
                                </div>

                                {tip.examples.map((ex, exIdx) => {
                                  return (
                                    <div key={exIdx} className="bg-slate-50 dark:bg-slate-950/70 p-3 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2">
                                      {ex.wrong && (
                                        <div className="flex items-start gap-2 text-rose-800 dark:text-rose-300 font-medium">
                                          <XCircle className="w-4 h-4 text-rose-600 dark:text-rose-400 flex-shrink-0 mt-0.5" />
                                          <div>
                                            <span className="line-through text-rose-600/80 dark:text-rose-400/80 mr-1 font-bold">{language === 'en' ? '[Incorrect]' : '[오답 패턴]'}</span>
                                            <span>{renderBoldText(ex.wrong)}</span>
                                          </div>
                                        </div>
                                      )}

                                      <div className="flex items-start justify-between gap-2 text-emerald-800 dark:text-emerald-300 font-medium">
                                        <div className="flex items-start gap-2">
                                          <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
                                          <div>
                                            <span className="text-emerald-700 dark:text-emerald-400 font-black mr-1">{language === 'en' ? '[Correct]' : '[올바른 문장]'}</span>
                                        ...
                                        <button
                                          ...
                                          className="p-1.5 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-700 transition-all flex-shrink-0"
                                          title="TTS Audio"
                                        >
                            ...
                            {/* 💡 Pro Tip Box */}
                            {proTip && (
                              <div className="bg-indigo-50 dark:bg-indigo-500/10 p-3.5 rounded-xl border border-indigo-200 dark:border-indigo-500/20 text-indigo-950 dark:text-indigo-200">
                                <div className="text-[10px] font-black uppercase text-indigo-700 dark:text-indigo-400 tracking-wider mb-1 flex items-center gap-1">
