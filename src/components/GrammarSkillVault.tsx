...
export const GrammarSkillVault: React.FC = () => {
  const { language, t } = useLanguage();
  const [selectedCategoryId, setSelectedCategoryId] = useState<string>('all');
  const [searchQuery, setSearchQuery] = useState<string>('');
  const [expandedTipIds, setExpandedTipIds] = useState<Record<string, boolean>>({
    article_noun: true, // open the first one by default as an example
  ...
  return (
    <div className="mt-8 pt-8 border-t border-slate-700/80 text-left">
      ...
      <div className="bg-gradient-to-r from-amber-500/15 via-purple-500/15 to-indigo-500/15 rounded-3xl p-5 sm:p-7 border border-amber-500/30 mb-6 relative overflow-hidden shadow-xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black tracking-wider uppercase mb-2 bg-amber-500/20 text-amber-300 border border-amber-500/40">
              <Zap className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
              <span>{language === 'en' ? 'SECRET GRAMMAR CHEAT SHEET' : '실전 영문법 1초 정답 치트키'}</span>
            </div>
            <h3 className="text-xl sm:text-2xl font-black text-white flex items-center gap-2">
              <span>{language === 'en' ? 'Grammar Pro Skill & Hacks Vault 🧠' : '시험장 1초 킬러 문법 스킬 보관소 🧠'}</span>
            </h3>
            <p className="text-slate-300 text-xs sm:text-sm font-medium mt-1">
              {language === 'en'
                ? 'Master indispensable slot formulas, verb forms, and trap points to solve questions instantly!'
                : '관사 뒤 명사 자리, 감각동사 형용사 보어, 분사 구별 등 헷갈리는 문법 문제를 1초 만에 풀어내는 핵심 비법 모음집!'}
            </p>
          </div>

          <div className="flex-shrink-0">
            <span className="bg-slate-900/90 text-amber-400 border border-amber-500/30 px-3.5 py-2 rounded-2xl text-xs font-black flex items-center gap-1.5 shadow-md">
              <BookmarkCheck className="w-4 h-4 text-amber-400" />
              <span>{language === 'en' ? '12 Master Rules Included' : '총 12가지 특급 공식 수록'}</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 mb-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-slate-900/90 border border-slate-700/90 rounded-2xl text-white text-xs sm:text-sm placeholder-slate-500 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-all font-medium"
          />
          {searchQuery && (
            <button
