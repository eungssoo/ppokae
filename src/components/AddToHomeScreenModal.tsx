    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in zoom-in duration-200">
      <div className="bg-slate-900 border-2 border-indigo-500/60 rounded-[2.5rem] p-6 sm:p-8 max-w-md w-full relative shadow-2xl overflow-hidden text-center">
        ...
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider bg-indigo-500/20 text-indigo-300 border border-indigo-500/40 mb-2">
          ...
          <span>{language === 'en' ? 'Official App Free Install' : '정식 앱 무료 다운로드 & 설치'}</span>
        </div>

        <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight mb-2">
          {language === 'en' ? 'Launch in 1 Second with App' : '스마트폰 앱으로 1초 만에 실행'}
        </h3>

        <p className="text-slate-300 text-xs sm:text-sm leading-relaxed mb-4">
          {language === 'en' ? (
            <>
              Enjoy a <strong>clean fullscreen view without browser bars</strong>.<br />
              Master English daily right from your home screen!
            </>
          ) : (
            <>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4 text-left">
          <div className="p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700 text-center">
            ...
            <span className="text-[11px] font-black text-white block">{language === 'en' ? '1s Launch' : '1초 실행'}</span>
            <span className="text-[9px] text-slate-400">{language === 'en' ? 'Home Icon' : '바탕화면 원터치'}</span>
          </div>
          <div className="p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700 text-center">
            <Smartphone className="w-4 h-4 text-indigo-400 mx-auto mb-1" />
            <span className="text-[11px] font-black text-white block">{language === 'en' ? 'Fullscreen' : '풀스크린'}</span>
            <span className="text-[9px] text-slate-400">{language === 'en' ? 'No URL Bar' : '주소창 100% 제거'}</span>
          </div>
          <div className="p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700 text-center">
            <CalendarCheck className="w-4 h-4 text-pink-400 mx-auto mb-1" />
            <span className="text-[11px] font-black text-white block">{language === 'en' ? 'App Sync' : '자동 연결'}</span>
            <span className="text-[9px] text-slate-400">{language === 'en' ? 'Native feel' : '설치 시 앱으로 실행'}</span>
          </div>
        </div>
        ...
        {deferredPrompt && (
          ...
          >
            ...
            <span>{language === 'en' ? '✨ Install App Now' : '✨ 지금 바로 스마트폰 앱 설치하기'}</span>
          </button>
        )}

        {/* 📋 Device & Browser Step-by-Step Guide */}
        {deviceType === 'ios' ? (
          /* 🍏 iOS Safari Guide */
          <div className="p-4 rounded-2xl bg-slate-800/90 border border-pink-500/40 text-left space-y-2 shadow-inner mb-4">
            <span className="text-xs font-black text-pink-300 flex items-center gap-1.5">
              <span>🍏 {language === 'en' ? 'iPhone (iOS Safari) Guide' : '아이폰(iOS Safari) 설치 방법'}</span>
            </span>
            <div className="space-y-2 text-xs text-slate-200">
              <div className="flex items-center gap-2.5 p-2 rounded-xl bg-slate-900/60 border border-slate-700/60">
                ...
                <span>{language === 'en' ? <>Tap the <strong>Share icon (<Share className="w-3.5 h-3.5 inline text-pink-300 mx-0.5" />)</strong> in Safari</> : <>사파리 화면 하단 <strong>공유 아이콘 (<Share className="w-3.5 h-3.5 inline text-pink-300 mx-0.5" />)</strong> 터치</>}</span>
              </div>
              <div className="flex items-center gap-2.5 p-2 rounded-xl bg-slate-900/60 border border-slate-700/60">
                ...
                <span>{language === 'en' ? <>Select <strong>[Add to Home Screen <PlusSquare className="w-3.5 h-3.5 inline text-pink-300 mx-0.5" />]</strong></> : <>메뉴에서 <strong>[홈 화면에 추가 <PlusSquare className="w-3.5 h-3.5 inline text-pink-300 mx-0.5" />]</strong> 선택</>}</span>
              </div>
            </div>
          </div>
        ) : (
          ...
          <div className="p-4 rounded-2xl bg-slate-800/90 border border-indigo-500/40 text-left space-y-2.5 shadow-inner mb-4">
            ...
            <div className="space-y-2 text-xs text-slate-200">
              <div className="flex items-center gap-2.5 p-2 rounded-xl bg-slate-900/60 border border-slate-700/60">
                ...
                <span>
                  {language === 'en' ? <>Tap the <strong>[Install <Download className="w-3 h-3 inline text-indigo-300 mx-0.5" />]</strong> icon in the address bar</> : <>브라우저 <strong>상단 주소창 우측</strong>의 <strong>[앱 설치 <Download className="w-3 h-3 inline text-indigo-300 mx-0.5" />]</strong> 또는 <strong>[⊕]</strong> 아이콘 터치</>}
                </span>
              </div>
              <div className="flex items-center gap-2.5 p-2 rounded-xl bg-slate-900/60 border border-slate-700/60">
                ...
                <span>{language === 'en' ? 'Tap [Install] on the prompt to add to home screen!' : '하단 팝업에서 [설치]를 누르면 1초 만에 스마트폰 앱으로 등록 완료!'}</span>
              </div>
            </div>
          </div>
        )}
        ...
        >
          {language === 'en' ? 'Maybe later' : '다음에 할게요'}
        </button>
