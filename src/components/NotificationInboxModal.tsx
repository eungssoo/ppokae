...
export const NotificationInboxModal: React.FC<NotificationInboxModalProps> = ({
  ...
  const BADGE_CONFIG = language === 'en' ? {
    event: { label: '🎉 Event', color: 'bg-pink-500/20 text-pink-300 border-pink-500/40' },
    notice: { label: '📢 Notice', color: 'bg-blue-500/20 text-blue-300 border-blue-500/40' },
    update: { label: '⚡ Update', color: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' },
    maintenance: { label: '🛠️ Maintenance', color: 'bg-amber-500/20 text-amber-300 border-amber-500/40' }
  } : {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-black text-white flex items-center gap-2">
                <span>{language === 'en' ? 'Inbox & News' : '알림 & 소식 보관함'}</span>
              </h3>
              <p className="text-xs text-slate-400">
                {language === 'en' ? 'Latest announcements, events & gifts' : 'PPOKAE 최신 공지, 이벤트 및 선물 수령'}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 py-4 pr-1 custom-scrollbar relative z-10">
          {announcements.length === 0 ? (
            <div className="py-16 text-center text-slate-500 space-y-2">
              ...
              <p className="text-sm font-bold">{language === 'en' ? 'No new announcements.' : '새로운 알림이 없습니다.'}</p>
              <p className="text-xs">{language === 'en' ? 'New notices will be saved here.' : '새 소식이 도착하면 이곳에 차곡차곡 보관됩니다.'}</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] text-slate-500 font-mono flex items-center gap-1">
                      ...
                      <span>{new Date(item.createdAt).toLocaleDateString(language === 'en' ? 'en-US' : 'ko-KR', { month: 'numeric', day: 'numeric' })}</span>
                    </span>
                  </div>
                  ...
                  {hasReward && (
                    <div className="pt-2.5 border-t border-slate-700/60 flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-xs text-yellow-300 font-black">
                        ...
                        <span>{language === 'en' ? `Gift: 🪙 +${item.rewardCoins} Coins` : `선물: 🪙 +${item.rewardCoins} 코인`}</span>
                      </div>
                      ...
                      {isClaimed ? (
                        <span className="px-3 py-1 rounded-xl bg-slate-800 text-slate-500 border border-slate-700 text-xs font-bold flex items-center gap-1">
                          ...
                          <span>{language === 'en' ? 'Claimed' : '수령 완료'}</span>
                        </span>
                      ) : (
                        <button
                        ...
                        >
                          ...
                          <span>{claimingIds[item.id] ? (language === 'en' ? 'Claiming...' : '수령 처리 중...') : (language === 'en' ? `Claim 🪙 ${item.rewardCoins} Coins` : `🪙 ${item.rewardCoins} 코인 받기`)}</span>
                        </button>
                      )}
                    </div>
                  )}
